--
-- File generated with SQLiteStudio v3.2.1 on Wed Jan 1 12:07:08 2020
--
...
CREATE TABLE District (ID INTEGER PRIMARY KEY ASC AUTOINCREMENT CHECK (ID = 1) DEFAULT (1), App_ID INTEGER REFERENCES App (ID) ON DELETE CASCADE ON UPDATE CASCADE UNIQUE NOT NULL, Name TEXT UNIQUE NOT NULL, Grade_Span_From STRING, Grade_Span_To STRING, Abbreviation STRING, AD_FQDN STRING, AD_Server STRING, AD_NetBIOS STRING, AD_Username STRING, AD_Password, GA_FQDN STRING, Parent_Email_Group STRING, Staff_Username_Format STRING, Student_Username_Format STRING, Using_GADS BOOLEAN NOT NULL DEFAULT (FALSE), Using_GAPS BOOLEAN NOT NULL DEFAULT (FALSE));
...
CREATE TABLE GoogleApps (ID INTEGER PRIMARY KEY AUTOINCREMENT, School_ID INTEGER REFERENCES School (ID) ON DELETE CASCADE ON UPDATE CASCADE, District_ID INTEGER REFERENCES District (ID) ON DELETE CASCADE ON UPDATE CASCADE, Department_ID INTEGER REFERENCES Department (ID) ON DELETE CASCADE ON UPDATE CASCADE, Grade_ID INTEGER REFERENCES Grade (ID) ON DELETE CASCADE ON UPDATE CASCADE, Team_ID INTEGER REFERENCES Team ON DELETE CASCADE ON UPDATE CASCADE, Type STRING NOT NULL CHECK ((Type = 'Student' or Type = 'Staff')), OU STRING, "Group" STRING, Username_Format STRING, Other_Groups STRING, Force_Password_Change BOOLEAN NOT NULL DEFAULT (TRUE));
